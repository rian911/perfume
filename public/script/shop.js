...
function updateDisplayedProduct() {
  ...
  if (selectedFragrance.length === 0) {
    ...
    filteredProducts = product.filter((product) =>
      product.type.some((fragrance) => selectedFragrance.includes(fragrance))
    );
  }
  ...
  filteredProducts.forEach((product) => {
    shopHTML += ` <div class="xl:w-60  bg-pm1 lg:w-2/5  p-4 m-1 rounded-xl w-full max-h-[345px] transform transition-all hover:-translate-y-2 duration-300 shadow-lg hover:shadow-2xl hover:shadow-sec1">
...
fragranceCheckboxes.forEach((checkbox) => {
  checkbox.addEventListener('change', updateDisplayedProduct);
});
